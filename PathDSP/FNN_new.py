...
def main(params):
    ...
    all_ytest_df.to_csv(params['output'] + '.FNN.cv_' + str(params['cv_int']) + '.Prediction.txt', header=True, index=True, sep="\t")
    all_loss_df.to_csv(params['output'] + '.FNN.cv_' + str(params['cv_int']) + '.Loss.txt', header=True, index=False, sep="\t")
    if params['shap_bool'] == True:
    ...
    tch.save(best_model.state_dict(), params['output'] + '.FNN.cv_' + str(params['cv_int']) + 'best_model.pt')
