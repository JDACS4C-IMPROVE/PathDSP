...
# swtich from improve_utils to improve repo
...
def initialize_parameters():
    preprocessor_bmk = PathDSP_candle(
        file_path,
        "PathDSP_params.txt",
        "pytorch",
        prog="PathDSP_candle",
...
def preprocess(params, data_dir):
    ...
    for i in [
        ...
        params["x_data_path"] = (
            os.environ["CANDLE_DATA_DIR"] + "/csa_data/raw_data/x_data/"
        )
    return params


def download_anl_data(params):
    csa_data_folder = os.path.join(
        os.environ["CANDLE_DATA_DIR"], "csa_data", "raw_data"
    )
    ...
    for improve_file in [
        ...
        candle.file_utils.get_file(
            improve_file, url_dir + improve_file, datadir=splits_dir, cache_subdir=None
        )
...
def download_author_data(params):
    data_download_filepath = candle.file_utils.get_file(
        params["original_data"],
        params["original_data_url"] + "/" + params["original_data"],
        datadir=params["data_dir"],
        cache_subdir=None,
    )
    ...
    random_seed = 42
    df = pd.read_csv(
        params["data_dir"] + "/input.txt", sep="\t"
    )  # Modify the separator if needed
    df = df.set_index(["drug", "cell"])
    train_data, temp_data = skms.train_test_split(
        df, test_size=0.2, random_state=random_seed
    )
    val_data, test_data = skms.train_test_split(
        temp_data, test_size=0.5, random_state=random_seed
    )
    pl.from_pandas(train_data).write_csv(
        params["train_data"], separator="\t", has_header=True
    )
    pl.from_pandas(val_data).write_csv(
        params["val_data"], separator="\t", has_header=True
    )
    pl.from_pandas(test_data).write_csv(
        params["test_data"], separator="\t", has_header=True
    )


def load_smiles_data(fname: str, sep: str = "\t", verbose: bool = True) -> pd.DataFrame:
    """
...
def smile2bits(params):
    ...
    response_df = [
        drp.load_response_data(
            y_data_fpath=Path(
                params["CANDLE_DATA_DIR"] + "/csa_data/raw_data/y_data/response.tsv"
            ),
            split_fpath=Path(
                params["CANDLE_DATA_DIR"] + "/csa_data/raw_data/splits/" + params[file]
            ),
        )
        for file in ["train_split_file", "test_split_file", "val_split_file"]
    ]
    response_df = pd.concat(response_df, ignore_index=True)
    # smile_df = improve_utils.load_smiles_data()
    # params['x_data_path'] = params['CANDLE_DATA_DIR'] + '/x_data/'
    # dd = drp.DrugsLoader(params)
    smile_df = load_smiles_data(
        fname=params["CANDLE_DATA_DIR"] + "/csa_data/raw_data/x_data/drug_SMILES.tsv"
    )
...
def run_netpea(params, dtype, multiply_expression):
    ...
    pathway_path = (
        params["data_dir"] + "/MSigdb/union.c2.cp.pid.reactome.v7.2.symbols.gmt"
    )
    log_transform = False
    ...
    if dtype == "DGnet":
        ...
        target_info = pd.read_csv(
            params["data_dir"] + "/data/DB.Drug.Target.txt", sep="\t"
        )
        ...
        mutation_data = drp.load_omics_data(
            params,
            omics_type="mutation_count",
...
def prep_input(params):
    ...
    for i in ["train_split_file", "test_split_file", "val_split_file"]:
        # response_df = improve_utils.load_single_drug_response_data(source=params['data_type'], split=params['split'],
        #                                                    split_type=i,
        #                                                    y_col_name= params['metric'])
        response_df = drp.load_response_data(
            y_data_fpath=Path(
                params["CANDLE_DATA_DIR"] + "/csa_data/raw_data/y_data/response.tsv"
            ),
            split_fpath=Path(
                params["CANDLE_DATA_DIR"] + "/csa_data/raw_data/splits/" + params[i]
            ),
        )
        response_df = response_df.rename(
            columns={"improve_chem_id": "drug_id", "improve_sample_id": "sample_id"}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="""whether to perform preprocessing using anl data or directly use processed 
